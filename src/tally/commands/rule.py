...
from tally.cli_utils import resolve_config_dir
...
def _find_rules_file(args) -> Tuple[Path, Optional[Path]]:
    ...
    config_path = resolve_config_dir(args, required=False)
    if config_path:
...
def _load_transactions(config_dir: Optional[Path]) -> List[Dict]:
    ...
    try:
        ...
        transforms = get_transforms(merchants_file, match_mode=rule_mode)
        all_txns = []

        for source in data_sources:
            source_files, _ = resolve_data_source_paths(str(config_dir), source.get('file'))
            if not source_files:
                continue
